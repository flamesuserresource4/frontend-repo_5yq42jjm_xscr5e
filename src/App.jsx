...
function SectionTitle({ eyebrow, title, subtitle }) {
  return (
    <div className="mb-10 text-center">
      {eyebrow && (
        <p className="text-xs uppercase tracking-widest text-violet-600 font-semibold mb-2">{eyebrow}</p>
      )}
  ...
  return (
    <div className="min-h-screen bg-white text-gray-900">
      ...
      <header className="fixed top-0 inset-x-0 z-50 bg-white/70 backdrop-blur-md border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="h-16 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <div className="size-8 bg-gradient-to-tr from-violet-600 to-blue-500 rounded-md" />
              <span className="text-sm font-semibold tracking-wider">Software Engineer</span>
            </div>
            <nav className="hidden md:flex items-center gap-8 text-sm">
              {NAV.map((item) => (
                <button key={item.id} onClick={() => handleNav(item.id)} className="text-gray-600 hover:text-gray-900">
                  {item.label}
                </button>
              ))}
            </nav>
            <button onClick={() => setMenuOpen((v) => !v)} className="md:hidden p-2 rounded-md hover:bg-gray-100" aria-label="Toggle menu">
        ...
        {menuOpen && (
          <div className="md:hidden border-t border-gray-200">
            <nav className="px-4 py-3 grid gap-2 bg-white/95">
              {NAV.map((item) => (
                <button key={item.id} onClick={() => handleNav(item.id)} className="text-left px-3 py-2 rounded-md hover:bg-gray-100">
                  {item.label}
                </button>
              ))}
            </nav>
      ...
      <section className="relative pt-24" id="hero">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-8 items-center">
            <div className="py-12">
              <p className="text-xs uppercase tracking-widest text-violet-600 font-semibold">Hi, I’m</p>
              ...
              <div className="mt-8 flex items-center gap-3">
                <a href="#projects" onClick={(e) => { e.preventDefault(); handleNav('projects') }} className="inline-flex items-center justify-center px-4 py-2 rounded-md bg-gray-900 text-white text-sm font-semibold hover:bg-gray-800">View Projects</a>
                <a href="#contact" onClick={(e) => { e.preventDefault(); handleNav('contact') }} className="inline-flex items-center justify-center px-4 py-2 rounded-md bg-gray-100 text-gray-900 text-sm font-semibold hover:bg-gray-200">Contact</a>
              </div>
            </div>
            <div className="h-[460px] sm:h-[520px] lg:h-[560px] rounded-xl overflow-hidden border border-gray-200 bg-white/60">
      ...
      <section id="skills" className="py-20 bg-gradient-to-b from-gray-50 to-white">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
            {['JavaScript', 'TypeScript', 'React', 'Next.js', 'Node.js', 'Python', 'FastAPI', 'Django', 'MongoDB', 'PostgreSQL', 'Docker', 'AWS', 'CI/CD', 'Testing', 'Tailwind CSS', 'Framer Motion'].map((s) => (
              <div key={s} className="flex items-center justify-between rounded-lg border border-gray-200 bg-white p-3 text-sm">
                <span>{s}</span>
      ...
      <section id="experience" className="py-20">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="space-y-4">
            {[
              { role: 'Senior Software Engineer', company: 'Tech Corp', period: '2023 — Present', points: ['Led development of a multi-tenant SaaS platform', 'Improved page performance by 40% with strategic caching', 'Mentored a team of 4 engineers'] },
              { role: 'Full‑Stack Engineer', company: 'Startup Labs', period: '2021 — 2023', points: ['Built and maintained GraphQL APIs', 'Implemented CI pipelines and automated tests', 'Collaborated with design to ship polished UI'] },
              { role: 'Software Engineer', company: 'Freelance', period: '2019 — 2021', points: ['Delivered end‑to‑end web apps for clients', 'Optimized apps for accessibility and SEO'] },
            ].map((job, idx) => (
          ...
          {loadingProjects ? (
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {(projects.length ? projects : fallbackProjects).map((p, i) => (
                <article key={(p.id || p._id || i)} className="group overflow-hidden rounded-xl border border-gray-200 bg-white">
                  <div className="h-40 bg-gradient-to-tr from-violet-100 to-blue-100" />
                  <div className="p-5">
                    ...
                    {p.tech && (
                      <div className="mt-3 flex flex-wrap gap-2">
                        {p.tech.slice(0, 4).map((t, idx) => (
                          <span key={idx} className="rounded bg-gray-100 px-2 py-0.5 text-xs text-gray-700 ring-1 ring-gray-200">{t}</span>
                        ))}
                      </div>
                    )}
                    <div className="mt-4 flex items-center gap-3 text-sm">
                      {p.url && <a href={p.url} target="_blank" rel="noreferrer" className="text-violet-600 hover:underline">Live</a>}
                      {p.repo && <a href={p.repo} target="_blank" rel="noreferrer" className="text-gray-700 hover:underline">Code</a>}
                    </div>
                  </div>
                </article>
              ))}
          ...
          <form onSubmit={onSubmitContact} className="grid grid-cols-1 gap-4">
            <div className="grid sm:grid-cols-2 gap-4">
              <div>
                ...
                <input name="name" required className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Email</label>
                <input type="email" name="email" required className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500" />
              </div>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Subject</label>
              <input name="subject" className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Message</label>
              <textarea name="message" required rows={5} className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500" />
            </div>
            <div className="flex items-center gap-3">
              <button disabled={contactState.loading} className="inline-flex items-center justify-center px-4 py-2 rounded-md bg-gray-900 text-white text-sm font-semibold hover:bg-gray-800 disabled:opacity-50">
      ...
      <footer className="py-10 border-t border-gray-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-sm text-gray-600 flex flex-wrap items-center justify-between gap-3">
